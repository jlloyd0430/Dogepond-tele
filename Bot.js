...
bot.on("message", (msg) => {
  if (msg.forward_from_chat) {
    console.log(
      "Forwarded message from channel. Channel ID:",
...
bot.onText(/\/setchannel (.+)/, (msg, match) => {
  ...
  bot.sendMessage(
    chatId,
    `Set the post channel to ${channelId} for ${dropType} drops`
  );
});
...
const formatPostMessage = (post) => {
  ...
  message += `*Date:* ${
    post.date === "TBA" ? "TBA" : new Date(post.date).toLocaleDateString()
  }\n`;
  ...
  if (post.dropType === "new mint") {
    ...
    message += `*Whitelist Price:* ${
      post.wlPrice !== undefined ? post.wlPrice : "N/A"
    }\n`;
  } else if (post.dropType === "auction") {
    message += `*Starting Price:* ${
      post.startingPrice !== undefined ? post.startingPrice : "N/A"
    }\n`;
    message += `*Marketplace Link:* ${
      post.marketplaceLink ? `[Link](${post.marketplaceLink})` : "N/A"
    }\n`;
  } else if (post.dropType === "airdrop") {
    message += `*Project Link:* ${
      post.projectLink ? `[Link](${post.projectLink})` : "N/A"
    }\n`;
  }
        ...
        if (latestPost._id !== latestPostId) {
          ...
          for (const chatId in channelConfig) {
            ...
            if (dropType !== "any" && latestPost.dropType !== dropType) {
              console.log(
                `Skipping post of type ${latestPost.dropType} for chat ${chatId} with configured type ${dropType}`
              );
              continue;
            }
